...
//        loadPortfolioItemDetails()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.cryptoDetails.collectLatest(viewLifecycleOwner) { details ->
            val cId = details?.id ?: 0
            val cSymbol = details?.symbol ?: ""
            val cPrice = details?.quote?.usd?.price ?: 0.0
            val cChange24 = details?.quote?.usd?.percentChange24h ?: 0.0
            val change7d = details?.quote?.usd?.percentChange7d ?: 0.0
            val change30d = details?.quote?.usd?.percentChange30d ?: 0.0
            val change60d = details?.quote?.usd?.percentChange60d ?: 0.0
            val change90d = details?.quote?.usd?.percentChange90d ?: 0.0
            val cmcRank = details?.cmcRank ?: 0
            val marketCap = details?.quote?.usd?.marketCap ?: 0.0
//            val maxSupply = details?.max_supply ?: 0
            val circulatingSupply = details?.circulatingSupply ?: 0.0
            val totalSupply = details?.totalSupply ?: 0.0
            val marketPair = details?.numMarketPairs ?: 0
            val dilutedMarket = details?.quote?.usd?.dilutedMarketCap ?: 0.0
            val marketDominance = details?.quote?.usd?.marketCapDominance ?: 0.0
            val volume24h = details?.quote?.usd?.volume24h ?: 0.0
